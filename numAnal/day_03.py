...
def logistic():
    ...
    # 读取数据
    data = pd.read_csv("C:/Users/28670/Desktop/wc.txt", names=column)
    ...
    data_join = data.dropna()
    print(type(data))

    # 进行数据的分割
    x_train, x_test, y_train, y_test = train_test_split(data_join[column[0:10]], data_join[column[10]], test_size=0.25)

    print("test" ,x_test["Sample code number"])
    #print(type(x_test))
    #print(len(x_test))
    # 进行标准化处理
    std = StandardScaler()
